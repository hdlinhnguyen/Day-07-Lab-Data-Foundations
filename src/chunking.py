...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if all(len(part) <= self.chunk_size for part in parts):
            ...
            current_chunk = ''
            for part in parts:
                candidate = part if current_chunk == '' else current_chunk + separator + part
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        def stats(chunks: list[str], preserves: bool) -> dict:
            count = len(chunks)
            ...
            return {
                ...
                'avg_length': avg_len,
                'chunks': chunks,
                'preserves_context': preserves,
            }

        return {
            'fixed_size': stats(fixed.chunk(text), False),
            'by_sentences': stats(sentence.chunk(text), True),
            'recursive': stats(recursive.chunk(text), True),
        }
